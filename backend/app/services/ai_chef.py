...
def generate_recipes_ai(products: List[Dict], recipe_type: str = "general", max_budget: Optional[float] = None) -> List[Dict]:
    ...
    if not client:
        return generate_fallback_recipes(products, recipe_type, max_budget)
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": "Ești un bucătar profesionist român. Creezi DOAR rețete realiste, gustoase, care au sens culinar. Nu combini ingrediente incompatibile. Răspunzi DOAR cu JSON valid, fără markdown sau explicații."
                }, 
...
def generate_fallback_recipes(products: List[Dict], recipe_type: str = "general", max_budget: Optional[float] = None) -> List[Dict]:
    if not products:
        return []
    ...
    recipes.append({
        ...
        "cook_time": "20 min",
        "servings": 4,
        "estimated_cost": round(cost1, 2),
        "cost_per_serving": round(cost1/4, 2),
        "difficulty": "ușor",
        "nutrition": {"calories": 350, "protein": 20, "carbs": 25, "fat": 15, "fiber": 5},
        "tags": ["rapid", "economic"],
        "tips": "Poți adăuga smântână pentru mai multă cremozitate",
        "image_url": get_recipe_image(p1[0]["name"]),
        "id": 1,
        "generated_at": now
    })
    ...
    if len(products) > 3:
        ...
        recipes.append({
            ...
            "description": "Combinație savuroasă cu produse la ofertă",
            "ingredients": [
            ...
            "cook_time": "15 min",
            "servings": 3,
            "estimated_cost": round(cost2, 2),
            "cost_per_serving": round(cost2/3, 2),
            "difficulty": "foarte ușor",
            "nutrition": {"calories": 280, "protein": 15, "carbs": 30, "fat": 10, "fiber": 6},
            "tags": ["vegetarian", "rapid"],
            "tips": "Perfect pentru un prânz ușor",
            "image_url": get_recipe_image(p2[0]["name"] if p2 else "legume"),
            "id": 2,
            "generated_at": now
        })
    ...
    if len(products) > 6:
        ...
        recipes.append({
            ...
            "ingredients": [
                {"name": x["name"], "quantity": "200g", "price": x["new_price"], "from_offer": True} 
                for x in p3
            ] + [
            ...
            "cook_time": "25 min",
            "servings": 4,
            "estimated_cost": round(cost3, 2),
            "cost_per_serving": round(cost3/4, 2),
            "difficulty": "mediu",
            "nutrition": {"calories": 320, "protein": 18, "carbs": 35, "fat": 12, "fiber": 4},
            "tags": ["traditional", "satios"],
            "tips": "Se poate servi cu pâine proaspătă",
            "image_url": get_recipe_image(p3[0]["name"] if p3 else "mancare"),
            "id": 3,
            "generated_at": now
        })
    
    print(f"Fallback: {len(recipes)} retete generate din {len(products)} produse")
    return recipes[:3]


def get_recipe_image(name: str) -> str:
    n = name.lower()
    if any(x in n for x in ["pui", "piept"]):
        return "https://images.unsplash.com/photo-1598103442097-8b74394b95c6?w=400"
    if any(x in n for x in ["porc", "ceafa"]):
        return "https://images.unsplash.com/photo-1432139555190-58524dae6a55?w=400"
    if any(x in n for x in ["peste", "somon"]):
        return "https://images.unsplash.com/photo-1519708227418-c8fd9a32b7a2?w=400"
    if any(x in n for x in ["paste", "spaghete"]):
        return "https://images.unsplash.com/photo-1621996346565-e3dbc646d9a9?w=400"
    if any(x in n for x in ["orez", "pilaf"]):
        return "https://images.unsplash.com/photo-1516714435131-44d6b64dc6a2?w=400"
    if any(x in n for x in ["legume", "tocan"]):
